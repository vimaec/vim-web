...
export class ColorManager {
  ...
  private _hexToColor = new Map<number, ColorHandle>()
  private _idToColor = new Map<number, ColorHandle>()

  private _toDelete : ColorHandle[] = []
   ...
   */
  async getColor (hex: RGBA32) : Promise<ColorHandle | undefined> {
  ...
  async getColors (c : RGBA32[]) {
    const result = new Array<ColorHandle>(c.length)
   ...
   * @private
   */
  private async _createColors (colors : RGBA32[]) : Promise<ColorHandle[] | undefined> {
    const result : ColorHandle[] = []
    if (colors.length === 0) {
      return result
    }
  ...
  private _createColor (color: RGBA32, id: number) {
    const handle = new ColorHandle(color, id, this)
   ...
   * @private
   */
  private _deleteColor (color: ColorHandle) {
